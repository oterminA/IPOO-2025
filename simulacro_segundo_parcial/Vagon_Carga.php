...
Es importante tener en cuenta que la variable de instancia que representa el peso del vagón se calcula de acuerdo a la cantidad de pasajeros que se está transportando y considerando un peso promedio por pasajero de 50 Kilos..
Si se trata de un vagón de carga se almacena el peso máximo que puede transportar y el peso de su carga
transportada. El peso del vagón va a depender del peso de su carga más un índice que coincide con un 20 % del peso de la carga, dicho índice se guarda en cada vagón de carga.
...
class Vagon_Carga extends Vagon
{
    ...
    //metodo constructor
    public function __construct($anio, $largo, $ancho, $pesoVacio, $pesoTransporte)
    {
        ...
        $this->indice=0.2; ///ya puesto como decimal
    ...
    //metodos nuevos
    /**
    * redefinicion de calcularPesoVagon()
    * El peso del vagón va a depender del peso de su carga más un índice que coincide con un 20 % del
peso de la carga, dicho índice se guarda en cada vagón de carga.
    */
    public function calcularPesoVagon(){
        ...
        $indice= $this->getIndice();
        $pesoCargaActualizado = $pesoPadre + ($pesoCarga + ($pesoCarga * $indice)); 
    ...
    public function incorporarCargaVagon($carga)
    {
        ...
        if ($sumaPesos <= $pesoMaximoTransporta) { //si la suma de ambos no excede el peso maximo que se puede transportar, hago:
            $this->setPesoCargaTransportada($sumaPesos); //actualizo el peso de la carga de la variable del padre
            $calculoPeso = $this->calcularPesoVagon(); //llamo a ese metodo
